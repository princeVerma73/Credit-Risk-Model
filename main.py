...
with row3[0]:
    delinquency_ratio=st.number_input("Delinquency Ratio",min_value=0,value=100,step=1)
with row3[1]:                                                      # value=30 → input box starts with 30 pre-filled
    credit_utilization_ratio=st.number_input('Credit Utilization Ratio',min_value=0,max_value=100,value=30,step=1)
with row3[2]:
    num_open_accounts=st.number_input("Open Loan Account",min_value=1,max_value=4,value=2,step=1)
